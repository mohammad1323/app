...
export function SnakeGameEnhanced() {
  ...
  const { stats, recordGame, getEquippedSnakeSkin, getEquippedFoodSkin } = useGameStore()
  const snakeSkin = getEquippedSnakeSkin()
  const foodSkin = getEquippedFoodSkin()
  ...
  const endGame = useCallback(() => {
    ...
    if (startTime) {
      ...
      recordGame(score, foodEaten, snake.length, playTime)
    }
  }, [startTime, score, foodEaten, snake.length, recordGame])
  ...
  useEffect(() => {
    ...
    const foodColor = foodSkin?.color || "#ff6b9d"
    const foodGlow = foodSkin?.glowColor || "#ff6b9d80"
    ...
    foodGradient.addColorStop(0, foodGlow)
    ...
    if (foodSkin?.shape === "circle") {
      ctx.beginPath()
      ctx.arc(foodCenterX, foodCenterY, CELL_SIZE / 2 - 2, 0, Math.PI * 2)
      ctx.fill()
    } else if (foodSkin?.shape === "diamond") {
      ctx.save()
      ctx.translate(foodCenterX, foodCenterY)
      ctx.rotate(Math.PI / 4)
      ctx.fillRect(-CELL_SIZE / 3, -CELL_SIZE / 3, CELL_SIZE / 1.5, CELL_SIZE / 1.5)
      ctx.restore()
    } else if (foodSkin?.shape === "star") {
      ctx.beginPath()
      for (let i = 0; i < 5; i++) {
        const angle = (i * 4 * Math.PI) / 5 - Math.PI / 2
        const x = foodCenterX + Math.cos(angle) * (CELL_SIZE / 2 - 2)
        const y = foodCenterY + Math.sin(angle) * (CELL_SIZE / 2 - 2)
    ...
    // Draw snake with equipped skin
    const headColor = snakeSkin?.headColor || "#22c55e"
    const bodyColor = snakeSkin?.bodyColor || "#22c55e"
    const glowColor = snakeSkin?.glowColor || "#22c55e80"
    ...
    snake.forEach((segment, index) => {
      ...
      // Snake body
      const segmentColor = isHead ? headColor : bodyColor
      ctx.fillStyle = isHead ? segmentColor : `${segmentColor}${Math.round(alpha * 255).toString(16).padStart(2, '0')}`
      ctx.beginPath()
      ctx.roundRect(
  ...
  }, [snake, food, gameState, score, snakeSkin, foodSkin])
    ...
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="flex w-full max-w-[400px] items-center justify-between px-2">
        <div className="flex flex-col">
          ...
          <span className="text-3xl font-bold tabular-nums" style={{ color: snakeSkin?.headColor || "#22c55e" }}>
            {score}
          </span>
        </div>
        <div className="flex flex-col items-end">
          <span className="text-xs uppercase tracking-wider text-muted-foreground">High Score</span>
          <span className="text-3xl font-bold text-accent tabular-nums">{stats.highScore}</span>
        </div>
      </div>

      {/* Game Canvas */}
      <div
        className="relative rounded-xl border-2 border-border bg-card p-2"
        style={{ boxShadow: `0 0 40px ${snakeSkin?.glowColor || "rgba(34,197,94,0.15)"}` }}
      >
        <canvas ref={canvasRef} width={GRID_SIZE * CELL_SIZE} height={GRID_SIZE * CELL_SIZE} className="rounded-lg" />
      </div>
      ...
      <div className="flex gap-3">
        {gameState === "playing" ? (
          ...
          <Button
            ...
            style={{ backgroundColor: snakeSkin?.headColor || "#22c55e" }}
          >
      ...
      <div className="text-center text-sm text-muted-foreground">
        <p className="hidden md:block">
          Use{" "}
          <kbd className="rounded border border-border bg-secondary px-1.5 py-0.5 font-mono text-xs">Arrow Keys</kbd> or{" "}
